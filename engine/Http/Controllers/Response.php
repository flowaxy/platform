    ...
    public function json($data, int $statusCode = 200): void
    {
        ...
        if (headers_sent($file, $line)) {
            if (function_exists('logError')) {
                logError('Response::json: Headers already sent', [
                    'file' => $file,
                    'line' => $line,
                ]);
            } else {
        ...
        if (function_exists('logDebug')) {
            logDebug('Response::json: Sending JSON response', [
        ...
        try {
            ...
            if (function_exists('logInfo')) {
                logInfo('Response::json: JSON response sent successfully', [
                    'status_code' => $statusCode,
                ]);
            }
        } catch (Exception $e) {
            if (function_exists('logError')) {
                logError('Response::json: JSON encoding error', [
                    'error' => $e->getMessage(),
                    'exception' => $e,
                ]);
            } else {
