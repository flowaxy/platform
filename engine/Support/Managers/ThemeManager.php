...
class ThemeManager extends BaseModule
{
            ...
            try {
                ...
                if ($slug && ! Validator::validateSlug($slug)) {
                    if (function_exists('logWarning')) {
                        logWarning("ThemeManager: Invalid active theme slug from database", ['slug' => $slug]);
                    } else {
                ...
                if (function_exists('logError')) {
                    logError('ThemeManager loadActiveTheme error', ['error' => $e->getMessage(), 'exception' => $e]);
                } else {
        ...
        if ($activeSlug) {
            ...
            if ($theme !== null && is_array($theme)) {
                ...
                if (function_exists('logDebug')) {
                    logDebug('ThemeManager: Active theme loaded', [
                        'theme' => $activeSlug,
    ...
    private function loadThemeSettings(string $themeSlug): void
    {
        if (! Validator::validateSlug($themeSlug)) {
            if (function_exists('logWarning')) {
                logWarning("ThemeManager: Invalid theme slug", ['theme_slug' => $themeSlug]);
            } else {
        ...
        $savedSettings = cache_remember($cacheKey, function () use ($themeSlug) {
            try {
                ...
                if (function_exists('logError')) {
                    logError('ThemeManager loadThemeSettings error', ['error' => $e->getMessage(), 'exception' => $e]);
                } else {
    ...
    public function getAllThemes(): array
    {
        ...
        if (! is_dir($themesDir)) {
            if (function_exists('logError')) {
                logError("ThemeManager: Themes directory not found", ['themesDir' => $themesDir]);
            } else {
            ...
            foreach ($directories as $dir) {
                ...
                if (file_exists($configFile) && is_readable($configFile)) {
                    ...
                    if ($configContent === false) {
                        if (function_exists('logWarning')) {
                            logWarning("ThemeManager: Cannot read theme.json", ['themeSlug' => $themeSlug]);
                        } else {
                    ...
                    if ($config && is_array($config)) {
                        ...
                        if (function_exists('logWarning')) {
                            logWarning("ThemeManager: Invalid JSON in theme.json", ['themeSlug' => $themeSlug]);
                        } else {
        ...
        try {
            ...
            return cache_remember($cacheKey, function () use ($themeSlug, $db): bool {
                ...
                try {
                    ...
                    if (function_exists('logError')) {
                        logError('ThemeManager isThemeActive error', ['error' => $e->getMessage(), 'exception' => $e]);
                    } else {
                        if (function_exists('logError')) {
                logError('ThemeManager isThemeActive error', ['error' => $e->getMessage(), 'exception' => $e]);
            } else {
    ...
    public function activateTheme(string $slug): bool
    {
        if (! Validator::validateSlug($slug)) {
            if (function_exists('logError')) {
                logError("ThemeManager: Invalid theme slug for activation", ['slug' => $slug]);
            } else {
        ...
        try {
            ...
            if (function_exists('logError')) {
                logError('ThemeManager activateTheme service error', ['error' => $e->getMessage(), 'exception' => $e]);
            } else {
    ...
    public function deactivateTheme(string $slug): bool
    {
        ...
        if (! Validator::validateSlug($slug)) {
            if (function_exists('logError')) {
                logError("ThemeManager: Invalid theme slug for deactivation", ['slug' => $slug]);
            } else {
        ...
        try {
            ...
            if (! $result) {
                if (function_exists('logError')) {
                    logError('ThemeManager: Failed to deactivate theme in database', ['slug' => $slug]);
                } else {
            ...
            if (function_exists('logError')) {
                logError('ThemeManager deactivateTheme error', ['error' => $e->getMessage(), 'exception' => $e]);
            } else {
    ...
    private function initializeDefaultSettings(string $themeSlug): bool
    {
        ...
        if (! Validator::validateSlug($themeSlug)) {
            if (function_exists('logError')) {
                logError("ThemeManager: Invalid theme slug for default settings", ['themeSlug' => $themeSlug]);
            } else {
        ...
        try {
                ...
                Database::getInstance()->transaction(function (PDO $db) use ($themeSlug, $defaultSettings): void {
                    foreach ($defaultSettings as $key => $value) {
                        if (empty($key) || ! Validator::validateString($key, 1, 255)) {
                            if (function_exists('logWarning')) {
                                logWarning("ThemeManager: Invalid default setting key", ['key' => $key]);
                            } else {
            ...
            if (function_exists('logError')) {
                logError('ThemeManager initializeDefaultSettings error', ['error' => $e->getMessage(), 'exception' => $e]);
            } else {
    ...
    public function getThemeSettings(string $themeSlug): array
    {
        if (! Validator::validateSlug($themeSlug)) {
            if (function_exists('logError')) {
                logError("ThemeManager: Invalid theme slug", ['themeSlug' => $themeSlug]);
            } else {
        ...
        $savedSettings = cache_remember($cacheKey, function () use ($themeSlug) {
            try {
                ...
                if (function_exists('logError')) {
                    logError('ThemeManager getThemeSettings error', ['error' => $e->getMessage(), 'exception' => $e]);
                } else {
    ...
    public function setSetting(string $key, $value): bool
    {
        ...
        if (! Validator::validateString($key, 1, 255)) {
            if (function_exists('logError')) {
                logError("ThemeManager: Invalid setting key", ['key' => $key]);
            } else {
        ...
        try {
            ...
            if (function_exists('logError')) {
                logError('ThemeManager setSetting service error', ['error' => $e->getMessage(), 'exception' => $e]);
            } else {
    ...
    public function setSettings(array $settings): bool
    {
        ...
        try {
            ...
            if (function_exists('logError')) {
                logError('ThemeManager setSettings service error', ['error' => $e->getMessage(), 'exception' => $e]);
            } else {
    ...
    public function getThemePath(?string $themeSlug = null): string
    {
        ...
        if (! Validator::validateSlug($slug)) {
            if (function_exists('logError')) {
                logError("ThemeManager: Invalid theme slug for path", ['slug' => $slug]);
            } else {
    ...
    public function getThemeUrl(?string $themeSlug = null): string
    {
        ...
        if (! Validator::validateSlug($slug)) {
            if (function_exists('logError')) {
                logError("ThemeManager: Invalid theme slug for URL", ['slug' => $slug]);
            } else {
    ...
    public function installTheme(string $slug, string $name, string $description = '', string $version = '1.0.0', string $author = ''): bool
    {
        ...
        if (! Validator::validateSlug($slug)) {
            if (function_exists('logError')) {
                logError("ThemeManager: Invalid theme slug for installation", ['slug' => $slug]);
            } else {
        ...
        if (! Validator::validateString($name, 1, 255)) {
            if (function_exists('logError')) {
                logError("ThemeManager: Invalid theme name", ['name' => $name]);
            } else {
        ...
        if (! $validation['valid']) {
            if (function_exists('logError')) {
                logError("ThemeManager: Theme structure validation failed", ['slug' => $slug, 'errors' => $validation['errors']]);
            } else {
        ...
        if (! empty($validation['warnings'])) {
            if (function_exists('logWarning')) {
                logWarning("ThemeManager: Theme structure warnings", ['slug' => $slug, 'warnings' => $validation['warnings']]);
            } else {
        ...
        try {
            ...
            if (function_exists('logError')) {
                logError('ThemeManager installTheme error', ['error' => $e->getMessage(), 'exception' => $e]);
            } else {
    ...
    public function getThemeConfig(?string $themeSlug = null): array
    {
        ...
        if (! Validator::validateSlug($slug)) {
            if (function_exists('logError')) {
                logError("ThemeManager: Invalid theme slug for config", ['slug' => $slug]);
            } else {
        ...
        return cache_remember($cacheKey, function () use ($slug, $theme, $themesBaseDir) {
            ...
            if (file_exists($jsonFile) && is_readable($jsonFile)) {
                try {
                    ...
                    if (function_exists('logError')) {
                        logError("ThemeManager: Error loading theme.json", ['error' => $e->getMessage(), 'exception' => $e, 'slug' => $slug]);
                    } else {
    ...
    public function getStylesheetUrl(?string $themeSlug = null, string $cssFile = 'style.css'): string
    {
        ...
        if ($this->hasScssSupport($theme['slug'])) {
            try {
                ...
                if (function_exists('logWarning')) {
                    logWarning('ThemeManager: SCSS compilation failed', ['error' => $e->getMessage(), 'exception' => $e]);
                } else {
    ...
    {
        if (function_exists('container')) {
            $repository = container()->make(ThemeSettingsRepositoryInterface::class);
    ...
    private function getEngineDir(): string
    {
        ...
        if ($engineDir === null) {
            // __DIR__ = engine/Support/Managers
            // dirname(__DIR__, 1) = engine/Support
            // dirname(__DIR__, 2) = engine
            // dirname(__DIR__, 3) = workbench (батьківська директорія engine)
            // Але нам потрібен саме engine, тому використовуємо dirname(__DIR__, 2)
    ...
    private function getRootDir(): string
    {
        static $rootDir = null;
        if ($rootDir === null) {
            // Спробуємо використати константу ROOT_DIR, якщо вона визначена
            if (defined('ROOT_DIR')) {
                $rootDir = ROOT_DIR;
            } else {
                // Якщо ROOT_DIR не визначена, використовуємо dirname(getEngineDir())
                // getEngineDir() повертає engine, тому dirname дає workbench
                $engineDir = $this->getEngineDir();
