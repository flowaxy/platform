...
class SecurityHelper
{
    ...
    {
        $securityClass = \Flowaxy\Core\Infrastructure\Security\Security::class;
        ...
        return $securityClass::csrfToken();
    }
    ...
    public static function verifyCsrfToken(?string $token = null): bool
    {
        ...
        return $securityClass::verifyCsrfToken($token);
    }
    ...
    public static function csrfField(): string
    {
        ...
        return $securityClass::csrfField();
    }
    ...
    public static function isAdminLoggedIn(): bool
    {
        ...
        try {
            $repository = class_exists('AdminUserRepositoryInterface')
                ? container()->make(AdminUserRepositoryInterface::class)
    ...
    public static function logout(): void
    {
        ...
        if ($userId > 0) {
            try {
                $logoutService = null;
                if (class_exists('LogoutAdminUserService')) {
                    $logoutService = function_exists('container')
                        ? container()->make(LogoutAdminUserService::class)
                        : new LogoutAdminUserService(new AdminUserRepository());
                }
    ...
    public static function safeHtml($value, string $default = ''): string
    {
        ...
        if (is_array($value) || is_object($value)) {
            return $securityClass::clean(Json::stringify($value));
        }

        return $securityClass::clean((string)($value ?: $default));
    }
    ...
    {
        $securityClass = \Flowaxy\Core\Infrastructure\Security\Security::class;
        $hasSecurity = class_exists($securityClass);

        if (!$hasSecurity && class_exists('Security')) {
            // Fallback для зворотної сумісності
            if (is_string($input)) {
        ...
        if (is_string($input)) {
            return $securityClass::clean(trim($input), true);
        }
